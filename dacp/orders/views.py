...
class CheckoutView(CartMixin, View):
        ...
        if cart.total_items == 0:
            if request.headers.get("HX-Request"):
                return TemplateResponse(
                    request, "orders/empty_cart.html", {"message": "Your cart is empty"}
                )
    ...
    def post(self, request):
        ...
        if not payment_provider or payment_provider not in ["stripe", "heleket"]:
            ...
            if request.headers.get("HX-Request"):
                return TemplateResponse(
                    request, "orders/checkout_content.html", context
                )
            return render(request, "orders/checkout.html", context)
        ...
        # Якщо не надав мейл, беремо з профілю якщо є
        ...
        if form.is_valid():
            ...
            # Спроба ініціалізації платіжної системи
            try:
            ...
            # Якщо є помилка, видаляємо щоб не забивати бд
                ...
                context = {
                    ...
                    "total_price": total_price,
                    "error_message": f"Payment processiong error: {str(e)}",
                }
                if request.headers.get("HX-Request"):
                    return TemplateResponse(
                        request, "orders/checkout_content.html", context
