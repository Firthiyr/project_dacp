...
class Cart:
    ...
    def add(self, product, size, quantity=1, override_quantity=False):
        ...
        cart_key = f"{product_id}_{size_name}"  # Цей блок для того, щоб відрізняти товари з різними розмірами
    ...
    def save(self):  # Сигнал що сесія була змінена і її потрібно зберегти
    ...
    def update_quantity(self, product, size, quantity):
        if quantity <= 0:
            ...
            return  # Якщо кількість 0 або менше, видаляємо товар з корзини
        else:
            self.add(product, size, override_quantity=quantity)

    def __iter__(self):
        product_ids = [
            item["product_id"] for item in self.cart.values()
        ]  # Збираємо id всіх товарів у корзині
    ...
    def __len__(self):  # Підрахунок всіх товарів у корзині
    ...
    def clear(self):
        del self.session["cart"]  # Видаляємо корзину зі сесії
        self.save()

    def get_cart_item(self):  # Для правильного редагування твоарів
        items = []
